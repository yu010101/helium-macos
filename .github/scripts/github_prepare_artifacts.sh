#!/bin/bash -eux
# Idaten: パイプの途中(tar)が失敗しても zstd の成功で見逃さないように
set -o pipefail
...
# Idaten: 既定の tar 形式は更新時刻の1秒未満を切り捨てる(手元で実測: ...582315301 ns → ...000000000 ns)。
# siso はナノ秒の更新時刻で「作り終えたか」を照合するので、切り捨てると次の段でほぼ全部を作り直す。pax 形式はナノ秒を保つ
tar --format=pax -C build -c -f - src | zstd -vv -11 -T0 -o build_src.tar.zst
...
if [ -n "${SCCACHE_DIR:-}" ] && [ -d "$SCCACHE_DIR" ]; then
  ...
  tar --format=pax -C "$(dirname "$SCCACHE_DIR")" -c -f - "$(basename "$SCCACHE_DIR")" | zstd -vv -3 -T0 -o sccache.tar.zst
